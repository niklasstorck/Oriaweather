...
type
  ...
  public
    ...
    SaveWeather, IgnoreRoofFile: Boolean;
    OK: Boolean;
      ...
      try
        ...
        try
         ...
         country := JSONValue.GetValue<TJSONObject>('sys')
          ...
          City := 'Ingen plats eller';
          country := 'land hittas här!';
        end;
...
begin
  ...
  if Roofstatus = ' öppet'  then
    ...
    // Om flaggan ignorera roof status file ska taket anses öppet
    // men detta meddelas enligt nedan.
  if IgnoreRoofFile then
   begin
     Roofstatus := ' Ignorerad!! Ingen garanti att taket är öppet.';
     Stmp :='0 ';
   end;
...
begin
  ...
  MainForm.Width  := Inifile.ReadInteger('Place', 'Width', 400);
  MainForm.Height := Inifile.ReadInteger('Place', 'Height', 211);
  MainForm.Top    := Inifile.ReadInteger('Place', 'Top', 20);
  MainForm.Left   := Inifile.ReadInteger('Place', 'Left', 20);
  ...
  IgnoreRoofFile  := IniFile.ReadBool('Settings', 'Ignore roof file',False);
  Inifile.Destroy
end;
...
begin
  try
    ...
    IniFile.WriteBool('Settings', 'Ignore roof file',IgnoreRoofFile);
...
begin
  ...
  try
    ...
    if IgnoreRoofFile then
    begin
      Memo1.Brush.Color := RGB(230, 230, 128);
      OK := true
    end
    else
      if OK  then
